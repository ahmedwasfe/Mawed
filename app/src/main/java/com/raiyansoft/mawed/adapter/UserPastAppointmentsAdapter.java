...
public class UserPastAppointmentsAdapter extends RecyclerView.Adapter<UserPastAppointmentsAdapter.UserOrdersHolder> {
    private static final String TAG = UserPastAppointmentsAdapter.class.getSimpleName();

    private Activity activity;
    private List<UserOrdersData> listDates;
    private LayoutInflater inflater;

    public UserPastAppointmentsAdapter(Activity activity, List<UserOrdersData> listDates) {
    ...
    public void onBindViewHolder(@NonNull UserOrdersHolder holder, int position) {
        ...
        if (order != null) {
            String title = new StringBuilder()
                    .append(activity.getString(R.string.an_date_with))
                    .append(" ")
            ...
            holder.setItemClickListener((view, pos) -> {
                ...
                bookingData.setLatitude(order.getLat());
                bookingData.setLongitude(order.getLng());
            ...
            holder.btnLocation.setOnClickListener(v -> {
                activity.startActivity(new Intent(activity, MapActivity.class)
                        .putExtra(Const.KEY_SALON_LATITUDE, order.getLat())
                        .putExtra(Const.KEY_SALON_LONGITUDE, order.getLng())
                        .putExtra(Const.KEY_SALON_NAME, order.getSalonName()));
            });
        }
    }
